"""
Continuous-Simple — intensidad continua desde D2.
...
def generate_plots(resultados_por_mh: dict, inst: dict, save_dir: str):
    ...
    ax1.set_title(f"Continuous-Simple — All MHs (seed={SEMILLA})")
    ...
    ax2.set_title(r"DTW Stagnation Signal — Intensity (Continuous-Simple, scale={})".format(SCALE))
    ...
    for ext in ("png", "pdf"):
        path = f"{save_dir}/continuous_simple_{inst_name}_{INDICE_INSTANCIA}.{ext}"
        fig.savefig(path)
        print(f"  Saved: {path}")
...
def main():
    ...
    print("=" * 70)
    print("  RESULTADOS MAESTRO — Continuous-Simple")
    print("=" * 70)
    ...
    print(f"  Continuous-Simple: scale={SCALE}")

    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    inst_name = Path(RUTA_INSTANCIA).stem
    save_dir = f"results/continuous_simple/todos/{inst_name}_{INDICE_INSTANCIA}/comparacion_mhs_{run_id}"
    Path(save_dir).mkdir(parents=True, exist_ok=True)
