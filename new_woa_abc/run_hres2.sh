#!/usr/bin/env bash
# Ejecución local/HPC de las siete variantes nuevas sobre HRES2.

#SBATCH --job-name=new_woaabc_hres2
#SBATCH --partition=CPU
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=7
#SBATCH --mem=16G
#SBATCH --output=/work/lucas.erazo/mkp_lb2/.hpc/logs/new_woaabc_hres2_%j.out
#SBATCH --error=/work/lucas.erazo/mkp_lb2/.hpc/errors/new_woaabc_hres2_%j.err
#SBATCH --qos=normal
...
mkdir -p .hpc/logs .hpc/errors .hpc/matplotlib new_woa_abc/resultados/hres2

WORKER_COUNT="${WORKERS:-${SLURM_CPUS_PER_TASK:-7}}"
RESUME_FLAGS=()
if [[ -n "${RESUME_DIR:-}" ]]; then
    RESUME_FLAGS+=("--resume-dir" "$RESUME_DIR")
fi

echo "New WOA--ABC HRES2 | workers=$WORKER_COUNT"
...
python3 -u -m new_woa_abc.runners.run_hres2 \
    --output-dir "${OUTPUT_DIR:-$PROJECT_DIR/new_woa_abc/resultados/hres2}" \
