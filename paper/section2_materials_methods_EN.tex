...
As an advanced real-world benchmark, the framework is applied to the sizing and operational dispatch optimization of a grid-connected Wind--Photovoltaic--Electrolyzer--Battery (WPEB) Microgrid with Green Hydrogen Production (HRES2-H2) located in Baotou, Inner Mongolia, China ($41.70^\circ\text{N}$, $110.43^\circ\text{E}$). The study extends the physical model of Li et al.~\cite{li2024capacity} by transforming the design domain into a mixed continuous-discrete optimization problem.
...
\toprule
\textbf{Variable} & \textbf{Symbol} & \textbf{Type} & \textbf{Search Domain} & \textbf{Physical Units / Description} \\
\midrule
Installed Wind Capacity & $P_{WT}$ & Continuous & $[0.0,\, 200.0]$ & MW \\
Installed PV Capacity & $P_{PV}$ & Derived & $200.0 - P_{WT}$ & MW \\
Electrolyzer Modules & $N_{el}$ & Integer & $\{10, 11, \dots, 20\}$ & Modules ($5\text{ MW/unit} \to [50, 100]\text{ MW}$) \\
Battery Power Capacity & $P_{bat}$ & Discrete & $\{0, 5, 10, \dots, 50\}$ & MW (5 MW increment steps) \\
Storage Duration & $\tau_{bat}$ & Discrete & $\{1.0,\, 2.0,\, 4.0\}$ & Hours of continuous nominal discharge \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:hres2_costs} lists the techno-economic cost parameters adopted from Li et al.~\cite{li2024capacity}.
