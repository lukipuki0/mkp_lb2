"""
mezclas_mh/woa_abc/mdg_wabc.py
------------------------------
MDG-WABC: Momentum Diversity-Guided WOA-ABC (Variante Combinada B + C).

Combina:
  - Criterio adaptativo de Diversidad Poblacional (Variante C) para decidir CUÁNDO cambiar de fase.
  - Momentum histórico + Paso adaptativo exponencial (Variante B) para guiado direccional en CÓMO moverse.
"""
...
class MDGWABCParams:
    ...
    umbral_init : float = 0.5    # Umbral de diversidad inicial
    umbral_final : float = 0.1   # Umbral de diversidad final
    # LB2 params
...
def ejecutar_epoch(
    ...
    for it in range(params.iterations):
        ...
        if div_norm <= umbral_t:
            # ══ FASE WOA + MOMENTUM (Población agrupada -> Explorar guiado) ══
            for i in range(pop_size):
            ...
            # ══ FASE ABC + MOMENTUM (Población dispersa -> Explotar guiado) ══
            for i in range(pop_size):
        ...
        if verbose:
            ...
            print(f"  [MDG-WABC MKP] Iter {it+1:3d}/{params.iterations} | Fase: {fase_str:<3} | DivNorm: {div_norm:.3f} | Mejor: {mejor_val:10.1f} | IterBest: {fit_iter_best:10.1f}")
        ...
        if monitor is not None:
            ...
            if status.get("fire"):
                stag_fires += 1
                if verbose:
                    print(f"    [MDG-WABC Stagnation] Fire #{stag_fires} @ iter {it} -> ABORT")
                break

    return MDGWABCEpochResult(
        epoch_idx = epoch_idx,
...
def ejecutar_epoch_continuo(
    ...
    for it in range(params.iterations):
        ...
        if verbose:
            fase_str = "WOA" if div_norm <= umbral_t else "ABC"
            print(f"  [MDG-WABC CEC2022] Iter {it+1:3d}/{params.iterations} | Fase: {fase_str:<3} | DivNorm: {div_norm:.3f} | Mejor(Min): {gbest_val:12.4f} | IterBest: {fit_iter_best:12.4f}")
        ...
        if monitor is not None:
            ...
            if status.get("fire"):
                stag_fires += 1
                if verbose:
                    print(f"    [MDG-WABC Continuous Stagnation] Fire #{stag_fires} @ iter {it} -> ABORT")
                break
...
if __name__ == "__main__":
    ...
    print("=== Demo MDG-WABC (Variante B + C) ===")
    
    # 1. MKP Discreto
    inst = MKPInstance.from_dict(cargar_instancias('instancias/mknapcb1.txt')[0])
    res_mkp = ejecutar_epoch(inst, MDGWABCParams(iterations=100), verbose=True)
    print(f"[MKP] Instancia (n={inst.n}, m={inst.m}) | Mejor Valor: {res_mkp.mejor_valor:.1f} | Iteraciones: {res_mkp.iteraciones}")

    # 2. Continuous CEC2022
    func = get_test_functions(10)[0]
    res_cont = ejecutar_epoch_continuo(func, MDGWABCParams(iterations=100), verbose=True)
    print(f"[CEC2022] Función (dim={func.n_dim}) | Mejor Valor (Min): {res_cont.mejor_valor:.4f} | Iteraciones: {res_cont.iteraciones}")
