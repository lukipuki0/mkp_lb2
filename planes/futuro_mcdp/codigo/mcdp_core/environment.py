...
class MCDP_Instance:
    ...
    def derive_part_assignments(self, assignments: Sequence[int]) -> np.ndarray:
        ...
        return np.argmax(self._part_cell_votes(values), axis=0).astype(int)

    def _part_cell_votes(self, assignments: np.ndarray) -> np.ndarray:
        """Return a ``cells × parts`` matrix with the majority-vote counts.

        This is algebraically equivalent to counting each part's processing
        machines cell by cell, but avoids a Python loop for every objective
        evaluation.  The cooperative batch evaluates millions of candidates,
        so keeping the same objective in matrix form materially reduces the
        runtime without changing tie handling or feasibility rules.
        """
        machine_cell = np.eye(self.max_cells, dtype=int)[assignments]
        return (self.incidence_matrix.T @ machine_cell).T
    ...
    def evaluate(self, assignments: Sequence[int]) -> MCDPEvaluation:
        ...
        votes = self._part_cell_votes(values)
        part_cells = np.argmax(votes, axis=0).astype(int)
        processing_counts = self.incidence_matrix.sum(axis=0)
        cost = int(np.sum(processing_counts - np.max(votes, axis=0)))
