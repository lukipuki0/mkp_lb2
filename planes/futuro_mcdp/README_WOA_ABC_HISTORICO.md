# WOA--ABC híbrido con DTW para MCDP y CEC2022
...
## Funciones continuas CEC2022

El mismo esquema está disponible para los descriptores de
`continuous_benchmark.funciones_cec2022` mediante
`cooperativo_cec_dtw.py`. La población continua compartida ejecuta WOA,
ABC empleada, ABC observadora y scout, con momentum y adaptación DTW.
...
funcion = get_test_functions(10)[0]  # CEC F1
resultado = ejecutar_cec_cooperativo(
    funcion,
    CooperativeCECParams(pop_size=30, iterations=100, seed=42),
    verbose=False,
)
print(resultado.mejor_valor_global, resultado.valor_optimo)
```

También puede usarse desde el orquestador continuo como `WOA-ABC`, por
ejemplo `ejecutar_mh_standalone(funcion, "WOA-ABC", max_iters=100)`.

## Ejecución por lote

El ejecutor `run_cooperative_mcdp_batch.py` replica el protocolo de
`MCDP-DL-DRL`: procesa las 10 matrices, con 2 celdas y capacidades 8--12, y
con 3 celdas y capacidades 6--9. Son 90 casos por corrida.

```bash
python -m woa_abc.run_cooperative_mcdp_batch
```

Cada corrida crea `woa_abc/results/run_TIMESTAMP/` con la tabla TXT, el CSV
compatible con el formato anterior, un JSON detallado y la configuración.
Los parámetros principales se pueden ajustar con
`--iterations`, `--epochs`, `--pop-size` y `--seed`.

Los óptimos publicados están guardados en `mcdp_optimos.csv`; el CSV/JSON de
cada corrida incluye `Optimum` y `Gap_Percent`, y también se genera una tabla
independiente `resultados_tabla_gap_CooperativeMCDP.txt`.

Para añadir la tabla de óptimos a una corrida existente:

```bash
python -m woa_abc.annotate_mcdp_results woa_abc/results/run_TIMESTAMP
```
...
```bash
python -m woa_abc.run_cooperative_mcdp --iterations 300
```
...
```powershell
python .\woa_abc\run_cooperative_mcdp.py --iterations 300
```
...
```bash
python -m woa_abc.run_cooperative_mcdp --file mcdp_core/instances/instancias.txt --instance 1 --cells 3 --capacity 6 --iterations 500 --epochs 3 --pop-size 30 --window 75 --band 0 --min-slope 0.1 --plateau-max 15 --patience 25 --ddtw --adapt --p-low 30 --p-high 70 --momentum-factor 0.2 --step-initial 1.0 --step-final 0.05 --seed 42
```
...
- `cooperativo_cec_dtw.py`: implementación continua para CEC2022.
