...
manejo de estancamiento en el Simulated Annealing (SA) para el MKP.
...
from sa_mkp import config
from sa_mkp.algorithm import SAParams, ejecutar_sa
...
def guardar_resumen_txt(resultado, label: str, output_dir: str) -> None:
    """Guarda un TXT con la tabla de resultados (igual a la consola) para la variante."""
    ruta = os.path.join(output_dir, f"resultado_{label}.txt")
    
    with open(ruta, "w", encoding="utf-8") as f:
        sep = "=" * 65
        f.write(f"{sep}\n")
        f.write(f"  RESULTADOS SA-MKP - VARIANTE: {label}\n")
        f.write(f"{sep}\n")
        f.write(
...
def graficar_convergencia_variante(resultado, label: str, output_dir: str, dpi: int = 150) -> None:
    ...
    ax.set_title(f"SA Convergence - MKP [{label}]", fontsize=14, fontweight="bold")
...
def main() -> None:
    ...
    output_dir = os.path.join("resultados", "resultados_sa", f"benchmark_{timestamp}")
    ...
    # Parametros base del SA
    base_params = SAParams(
        T_inicial            = config.T_INICIAL,
        T_final              = config.T_FINAL,
        alpha                = config.ALPHA,
        iter_por_T           = config.ITER_POR_T,
        num_flip             = config.NUM_FLIP,
        epochs               = config.EPOCHS,
        use_stagnation       = config.USE_STAGNATION,
        stag_cfg             = stag_cfg,
        stag_reheat_fraction = config.STAG_REHEAT_FRACTION,
    ...
    resultados_brutos = {}
    resultados_obj    = {}  # guardar SAResult completo para graficar

    # -- Ejecucion del Benchmark -----------------------------------------------
    print("\n" + "=" * 60)
    print(" STARTING SA-MKP VARIANT BENCHMARK")
    print("=" * 60)
    ...
    for label, strategy in variantes:
        ...
        # verbose=True para ver epochs y stagnation fires en consola
        resultado = ejecutar_sa(inst, params, verbose=True)
        ...
        # Exportar tabla de resultados a TXT
    ...
    ax.set_title("Result Distribution by Variant (SA-MKP)", fontsize=14, fontweight="bold")
