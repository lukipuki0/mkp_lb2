...
def ejecutar_epoch(
    ...
    Cuando detecta estancamiento sostenido dispara una de las estrategias:
      ...
      - "abort"   → termina el epoch inmediatamente (para el orquestador).

    Parameters
    ----------
    sol_inicial : list[int] | None
        Si se provee, el SA arranca desde esta solución (útil en el pipeline
        híbrido). Si es None, genera una solución aleatoria.
        ...
        if monitor is not None:
            ...
            if status.get("fire"):
                ...
                if max_fires > 0 and stag_fires >= max_fires:
                    ...
                    if params.stag_strategy == "abort":
                        if verbose:
                            print(f"    [Stagnation] Fire #{stag_fires} @ iter {iteraciones} -> ABORT (pipeline)")
                        break  # Sale del bucle while T > T_final

                    elif params.stag_strategy == "reheat":
