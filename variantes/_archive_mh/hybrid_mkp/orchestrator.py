...
from sa_mkp.algorithm  import SAParams,  ejecutar_epoch as _sa_epoch
from ts_mkp.algorithm  import TSParams,  ejecutar_epoch as _ts_epoch
from ga_mkp.algorithm  import GAParams,  ejecutar_epoch as _ga_epoch
from pso_mkp.algorithm import PSOParams, ejecutar_epoch as _pso_epoch
from gwo_mkp.algorithm import GWOParams, ejecutar_epoch as _gwo_epoch
...
def ejecutar_pipeline(
    ...
    if verbose:
        print("\n" + "=" * 62)
        print("  PIPELINE HÍBRIDO DTW — INICIO")
        print(f"  Tiempo máx : {tiempo_max}s")
        print(f"  Inyección  : {pop_injection_mode}")
    ...
    while (time.time() - t_inicio) < tiempo_max:
        ...
        if verbose:
            ...
            print(f"          Duración: {dur:.1f}s | iters: {n_iters} "
...
def _ejecutar_mh(
    ...
    """Ejecuta una MH específica con abort activado y semilla del orquestador."""
