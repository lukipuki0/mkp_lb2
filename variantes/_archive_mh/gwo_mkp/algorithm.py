"""
gwo_mkp/algorithm.py
--------------------
Motor principal del Grey Wolf Optimizer (GWO) para el MKP.

Implementa el bucle de optimizacion: inicializacion de la manada,
jerarquia Alpha/Beta/Delta, actualizacion de posiciones continuas,
binarizacion LB2, adaptacion de parametros G via DTW, y soporte
para 9 variantes de rescate de estancamiento.
"""
...
from gwo_mkp.rescues import (
    inyectar_heuristicos,
    mutacion_lp,
    ruin_and_recreate_manada,
)
...
class GWOEpochResult:
    """Resultado de una epoch del GWO."""
    epoch_idx        : int
    mejor_valor      : float
    iteraciones      : int
...
@dataclass
class GWOResult:
    """Resultado completo del GWO (todas las epochs)."""
...
def _obtener_jerarquia(
    fitnesses: list[float],
) -> tuple[int, int, int]:
    """Devuelve los indices de Alpha, Beta y Delta (los 3 mejores lobos)."""
...
def ejecutar_epoch(
    ...
    """Ejecuta una epoch completa del GWO y devuelve el resultado."""
    ...
    if sol_inyectada is not None:
        ...
        if mode == "random":
            ...
            # Mitad mutaciones, mitad aleatorios (ya creados)
            poblacion_bin[0] = sol_rep
            fitnesses[0] = val_rep
            ...
            for i in range(1, n_mutados):
                ...
                poblacion_bin[i] = msol
                fitnesses[i] = mval
            # El resto (desde n_mutados) ya es aleatorio de _inicializar_manada
    ...
    # Estado dinamico de los parametros G
    G1 = params.G1_i
    G2 = params.G2_i
    G3 = params.G3_i
    ...
    for it in range(params.iterations):
        ...
        for i in range(pop_size):
            # Los lideres no se actualizan por la manada
            if i in (alpha_idx, beta_idx, delta_idx):
                continue

            X_i = posiciones[i]

            # Coeficientes aleatorios para cada lider
            ...
            # Posiciones guiadas por los 3 lideres
            ...
            # Nueva posicion continua (promedio de las 3 guias)
            X_new = (X1 + X2 + X3) / 3.0
        ...
        if monitor is not None:
            ...
            # Adaptacion continua de G via DTW (siempre, si ready)
            ...
            # Verbose: imprimir metricas DTW por iteracion (estilo notebook)
            ...
            if status.get("fire"):
                ...
                if max_f > 0 and stag_fires >= max_f:
                    ...
                    if s == "abort":
                        break  # Sale del for it

                    elif s == "adapt_g":
                        # El Original: fuerza una exploracion temporal fuerte
                        G1 = params.G1_i
                        G2 = params.G2_i
                        G3 = params.G3_i
                        # Tambien reiniciar 30% peores lobos
                        n_reinicio = max(1, pop_size // 3)
                        peores = sorted(range(len(fitnesses)), key=lambda i: fitnesses[i])[:n_reinicio]
                        for idx in peores:
                            sol = [random.randint(0, 1) for _ in range(n)]
                            sol, val = reparar_solucion(sol, inst)
                            poblacion_bin[idx] = sol
                            fitnesses[idx]     = val
                            posiciones[idx]    = np.random.uniform(-params.v_max, params.v_max, size=n)

                    elif s == "v1_exploit":
                        # Forzar G a explotacion maxima
                        G1 = params.G1_f
                        G2 = params.G2_f
                        G3 = params.G3_f
                        # Reducir 'a' artificialmente en siguientes iters (ya esta cerca de 0 por la formula)
                        ...
                        poblacion_bin, fitnesses, posiciones = inyectar_heuristicos(
                            poblacion_bin, fitnesses, posiciones, inst, 0.50, params.v_max,
                        )

                    elif s == "v6_lp":
                        poblacion_bin, fitnesses = mutacion_lp(
                            poblacion_bin, fitnesses, inst, None,
                        )

                    elif s == "v7_tabu_lp":
                        poblacion_bin, fitnesses = mutacion_lp(
                            poblacion_bin, fitnesses, inst, tabu_genes,
                        )
                        ...
                        poblacion_bin, fitnesses = ruin_and_recreate_manada(
                            poblacion_bin, fitnesses, inst, 0.30,
                        )

                    # Recalcular jerarquia despues de un rescate
                    alpha_idx, beta_idx, delta_idx = _obtener_jerarquia(fitnesses)

                    # Restaurar G a default para estrategias que no son ciclicas
            ...
            # Sin monitor: transicion lineal ciega de G
            G1 = interpolar_G(it, params.iterations, params.G1_i, params.G1_f)
            G2 = interpolar_G(it, params.iterations, params.G2_i, params.G2_f)
            G3 = interpolar_G(it, params.iterations, params.G3_i, params.G3_f)
...
def ejecutar_gwo(
    ...
    """Ejecuta el GWO completo (todas las epochs) y retorna el GWOResult."""
    epochs_result    = []
