"""
pso_benchmark_variants.py
-------------------------
Script maestro para evaluar y comparar estadísticamente las 9 variantes de
manejo de estancamiento en el Particle Swarm Optimization (PSO) para el MKP.

Genera:
  - Salida verbose en consola (epochs, stagnation fires, etc.)
  - Un gráfico de convergencia por variante (convergencia_<label>.png)
  - Un boxplot comparativo global (distribucion_variantes.png)
  - Estadísticas (Media, Std, Wilcoxon) por consola
"""
...
from pso_mkp.algorithm import PSOParams, ejecutar_pso
...
def guardar_resumen_txt(resultado, label: str, output_dir: str) -> None:
    ...
    with open(ruta, "w", encoding="utf-8") as f:
        sep = "=" * 65
        f.write(f"{sep}\n")
        f.write(f"  RESULTADOS PSO-MKP - VARIANTE: {label}\n")
        f.write(f"{sep}\n")
        f.write(
        ...
        f.write(f"  Valor óptimo conocido    : {resultado.valor_optimo:.1f}\n")

        gap = resultado.gap_pct
        if gap is not None:
            f.write(f"  Gap relativo             : {gap:.2f}%\n")
        else:
            f.write("  Gap relativo             : N/A (óptimo no disponible en esta instancia)\n")
        f.write(f"{sep}\n")

    print(f"  [txt] Saved '{ruta}'")


# -- Gráfico de convergencia individual ----------------------------------------

def graficar_convergencia_variante(resultado, label: str, output_dir: str, dpi: int = 150) -> None:
    """Genera y guarda el gráfico de convergencia de una variante."""
    ...
    ax.set_title(f"PSO Convergence - MKP [{label}]", fontsize=14, fontweight="bold")
...
def main() -> None:
    ...
    output_dir = os.path.join("resultados", "resultados_pso", f"benchmark_{timestamp}")
    ...
    # Configuración de estancamiento base
    ...
    # Parámetros base del PSO
    base_params = PSOParams(
        pop_size       = 20, # config.POP_SIZE,
    ...
    print("\n" + "=" * 60)
    print(" STARTING PSO-MKP VARIANT BENCHMARK")
    print("=" * 60)
    ...
    for label, strategy in variantes:
        ...
        resultado = ejecutar_pso(inst, params, verbose=True)
    ...
    ax.set_title("Result Distribution by Variant (PSO-MKP)", fontsize=14, fontweight="bold")
